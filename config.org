...
(use-package lsp-mode
  ...
  (setq lsp-signature-function 'lsp-signature-posframe
        ...
        lsp-disabled-clients '((typescript-mode . vue-semantic-server))
        lsp-headerline-breadcrumb-enable nil))
#+end_src

This filters special characters which may otherwise cause the LSP client to log a specific error. See [[https://github.com/emacs-lsp/lsp-mode/issues/2681#issuecomment-1214902146][emacs-lsp/lsp-mode#2681]].

#+begin_src emacs-lisp
(defun lsp--json-parse-fix (orig &rest rest)
  (while (re-search-forward "\\u0000" nil t)
                (replace-match ""))
              (apply orig rest))

(advice-add 'json-parse-buffer :around #'lsp--json-parse-fix)
#+end_src
