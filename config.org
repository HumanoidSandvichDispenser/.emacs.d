...
This recipe allows ~org-roam~ to compile without fail.
...
(use-package org-roam
  ...
  (setq org-roam-capture-templates
      '(("m" "main" plain "%?"
        ...
        ("s" "study" plain "%?"
         :if-new
         (file+heada "study/${slug}.org" "#+title: ${title}\n")
         :immediate-finish t
         :unnarrowed t)
        ("f" "floating" plain "%?"
         :if-new
         (file+head "floating/${slug}.org" "#+title: ${title}\n")
         :immediate-finish t
         :unnarrowed t))))
#+end_src
...
Org to Hugo-compatible Markdown exporter
...
Set the default face font so fonts are consistent with PGTK builds

#+begin_src emacs-lisp
(set-face-attribute 'default nil :font "Iosevka Sandvich 12")
#+end_src
...
Use =C-c l= to copy link to the current location.
...
Then start a new tab with org-roam dailies.
